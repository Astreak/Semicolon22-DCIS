...
def windowing_image(read_img,lab):
  ...
  for i in range(0,512,4):
    for j in range(0,512,4):
      L.append([read_img[i:i+512,j+j:512,:],lab])
  print(len(L))
  return L
def create_image(Test,path):
  for i in os.listdir(path)[10:15]:
    path_tmp=os.path.join(path,i)
    read_img=imread(path_tmp)
    xshape=read_img.shape[1]
    yshape=read_img.shape[0]
...
plt.imshow(Test[0][0])
print(get_nuclei_den(Test[0][0]))

class CreateHeatmap(Dataset):
  def __init__(self,path,valid=False,transforms=None):
    self.path=path
    self.valid=valid
    self.transforms=transforms
    self.list={'DCIS':'/content/5_DCIS','Normal':'/content/drive/MyDrive/0_N'};
  def __len__(self):
    return self.path.__len__()
  def __getitem__(self,indx):
    name=self.path[indx];
    if self.valid:
      pp='/Images_val/5_DCIS_val'
    else:
      pp='/Images/5_DCIS';
    if 'DCIS' not in name:
      pp='/content/drive/MyDrive/0_N';
    image=os.path.join(pp,name);
    img_read=imread(image).astype(np.float32)
    img_read=img_read/255.
    fdim=img_read.shape[0];
    sdim=img_read.shape[1];
    
    if 'DCIS' not in name:
      labels=torch.tensor([0.],dtype=torch.float32)#change the labels
    else:
      labels=torch.tensor([1.],dtype=torch.float32)
    img_read=img_read.reshape((3,img_read.shape[0],img_read.shape[1]))
    img_read=torch.tensor(img_read)
    if self.transforms:
      img_read=self.transforms(img_read)
    return img_read,labels;